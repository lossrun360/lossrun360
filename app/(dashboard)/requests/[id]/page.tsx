...
export default function RequestDetailPage() {
  ...
  async function addManualPolicy() {
    if (!newPolicy.insurerName || !newPolicy.startDate) {
      toast.error('Insurer name and start date are required'); return
    }
    if (!params?.id) return
    try {
      const res = await fetch(`/api/requests/${params.id}/insurance`, {
        ...
        body: JSON.stringify({
          action: 'manual_add',
          carrierName: newPolicy.insurerName,
          policyType: COVERAGE_TYPE_MAP[newPolicy.coverageType] || newPolicy.coverageType,
          policyNumber: newPolicy.policyNumber || undefined,
          effectiveDate: newPolicy.startDate,
          cancellationDate: newPolicy.endDate || undefined,
        }),
      })
      if (!res.ok) {
          ...
          <div style={{ ...card, padding: '24px', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
              ...
              <div style={{ gridColumn: 'span 3' }}>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '600', color: '#64748b', marginBottom: '4px', textTransform: 'uppercase' as const, letterSpacing: '0.4px' }}>Address</label>
                <input value={editForm['address'] || ''} onChange={(e) => setEditForm(prev => ({ ...prev, address: e.target.value }))} style={{ width: '100%', padding: '8px 10px', border: '1px solid #e2e8f0', borderRadius: '4px', fontSize: '13px', color: '#0f172a', background: '#fff', outline: 'none', boxSizing: 'border-box' as const }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '600', color: '#64748b', marginBottom: '4px', textTransform: 'uppercase' as const, letterSpacing: '0.4px' }}>City</label>
                <input value={editForm['city'] || ''} onChange={(e) => setEditForm(prev => ({ ...prev, city: e.target.value }))} style={{ width: '100%', padding: '8px 10px', border: '1px solid #e2e8f0', borderRadius: '4px', fontSize: '13px', color: '#0f172a', background: '#fff', outline: 'none', boxSizing: 'border-box' as const }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '600', color: '#64748b', marginBottom: '4px', textTransform: 'uppercase' as const, letterSpacing: '0.4px' }}>State</label>
                <input value={editForm['state'] || ''} onChange={(e) => setEditForm(prev => ({ ...prev, state: e.target.value }))} maxLength={2} style={{ width: '100%', padding: '8px 10px', border: '1px solid #e2e8f0', borderRadius: '4px', fontSize: '13px', color: '#0f172a', background: '#fff', outline: 'none', boxSizing: 'border-box' as const }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '11px', fontWeight: '600', color: '#64748b', marginBottom: '4px', textTransform: 'uppercase' as const, letterSpacing: '0.4px' }}>Zip</label>
                <input value={editForm['zip'] || ''} onChange={(e) => setEditForm(prev => ({ ...prev, zip: e.target.value }))} style={{ width: '100%', padding: '8px 10px', border: '1px solid #e2e8f0', borderRadius: '4px', fontSize: '13px', color: '#0f172a', background: '#fff', outline: 'none', boxSizing: 'border-box' as const }} />
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ ...card, padding: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                ...
                <button onClick={() => setShowAddForm(!showAddForm)} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '12px', color: '#1c6edd', background: 'none', border: '1px solid #1c6edd', borderRadius: '3px', cursor: 'pointer', padding: '4px 10px', fontFamily: 'inherit', fontWeight: '500' }}>
                ...
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '4px', padding: '14px', marginBottom: '14px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    <div style={{ position: 'relative' }}>
                      ...
                      <input style={inp} placeholder="Search carriers..." value={newPolicy.insurerName} onChange={(e) => handleInsurerNameChange(e.target.value)} onBlur={() => setTimeout(() => setShowSuggestions(false), 150)} onFocus={() => newPolicy.insurerName.length >= 2 && searchCarriers(newPolicy.insurerName)} autoComplete="off" />
                      {showSuggestions && carrierSuggestions.length > 0 && (
                        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#fff', border: '1px solid #e2e8f0', borderRadius: '3px', boxShadow: '0 4px 12px rgba(15,23,42,0.12)', zIndex: 50, maxHeight: '200px', overflowY: 'auto' }}>
                          {carrierSuggestions.map((c) => (
                            <div key={c.id} onMouseDown={() => selectCarrierSuggestion(c)} style={{ padding: '8px 12px', cursor: 'pointer', borderBottom: '1px solid #f1f5f9' }} onMouseOver={(e) => (e.currentTarget.style.background = '#f8fafc')} onMouseOut={(e) => (e.currentTarget.style.background = '#fff')}>
                    ...
                    <div>
                      ...
                      <input style={inp} placeholder="e.g. POL-123456" value={newPolicy.policyNumber} onChange={(e) => setNewPolicy(prev => ({ ...prev, policyNumber: e.target.value }))} />
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: '600', color: '#64748b', marginBottom: '4px' }}>Start Date *</label>
                        <input style={inp} type="date" value={newPolicy.startDate} onChange={(e) => setNewPolicy(prev => ({ ...prev, startDate: e.target.value }))} />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '11px', fontWeight: '600', color: '#64748b', marginBottom: '4px' }}>End Date</label>
                        <input style={inp} type="date" value={newPolicy.endDate} onChange={(e) => setNewPolicy(prev => ({ ...prev, endDate: e.target.value }))} />
                      </div>
                    </div>
                  </div>
                  <div style={{ marginTop: '12px', display: 'flex', justifyContent: 'flex-end' }}>
                    <button onClick={addManualPolicy} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', padding: '6px 14px', background: '#1c6edd', color: '#fff', borderRadius: '3px', fontSize: '12px', fontWeight: '600', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>
                      Add Policy
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  {insurancePolicies.map((p, i) => (
                    <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1.5fr auto', gap: '8px', padding: '10px 0', borderBottom: i < insurancePolicies.length - 1 ? '1px solid #f1f5f9' : 'none', alignItems: 'center' }}>
                      ...
                      <button onClick={() => removePolicy(p)} title="Remove policy" style={{ width: '24px', height: '24px', border: 'none', background: 'transparent', cursor: 'pointer', color: '#94a3b8', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '3px', padding: 0 }} onMouseOver={(e) => (e.currentTarget.style.color = '#ef4444')} onMouseOut={(e) => (e.currentTarget.style.color = '#94a3b8')}>
                        &#10005;
                      </button>
                    </div>
                  ))}
