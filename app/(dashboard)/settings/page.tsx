...
export default function SettingsPage() {
  ...
  const card: React.CSSProperties = { background: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '24px' }
  const inp: React.CSSProperties = { width: '100%', padding: '8px 12px', border: '1px solid #e2e8f0', borderRadius: '6px', fontSize: '13px', color: '#0f172a', background: '#f8fafc', outline: 'none', boxSizing: 'border-box' }
      ...
      <div style={{ ...card }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
          {Object.values(PLANS).map((plan) => {
            ...
            return (
              <div key={plan.tier} style={{ background: '#fff', border: `1px solid ${isPopular ? '#6366f1' : '#e2e8f0'}`, borderRadius: '8px', padding: '20px', display: 'flex', flexDirection: 'column', position: 'relative', boxShadow: isPopular ? '0 0 0 3px rgba(99,102,241,0.12)' : 'none' }}>
                {isPopular && (
