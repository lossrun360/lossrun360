...
export function NewRequestModal({ isOpen, onClose }: Props) {
  ...
  const stepsConfig = [{ num: 1, label: 'DOT# Lookup' }, { num: 2, label: 'Review & Carriers' }, { num: 3, label: 'Send & Confirm' }]
            ...
            {stepsConfig.map((s, i) => (
              <div key={s.num} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                  <div style={{ width: '26px', height: '26px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '700', flexShrink: 0, background: step > s.num ? '#10b981' : step === s.num ? '#6366f1' : '#e2e8f0', color: step >= s.num ? '#fff' : '#64748b' }}>
                    {step > s.num ? '\u2713' : s.num}
                  </div>
                  <span style={{ fontSize: '12px', fontWeight: '500', color: step === s.num ? '#0f172a' : '#64748b', whiteSpace: 'nowrap' }}>{s.label}</span>
                </div>
                {i < stepsConfig.length - 1 && <div style={{ flex: 1, height: '1px', background: step > s.num ? '#10b981' : '#e2e8f0', margin: '0 10px' }} />}
              </div>
            ))}
          </div>
              ...
              <div style={card}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', fontSize: '13px' }}>
                  <div>
                    ...
                    <p style={{ color: '#64748b', margin: '0 0 1px', fontSize: '12px' }}>DOT# {dotInput}{form.mcNumber ? ` · ${form.mcNumber}` : ''}</p>
